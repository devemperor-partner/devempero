...
document.querySelector("#submit").onclick = () => {
  ...
  const deposit_amount = get_deposit_amount();
...
(() => {
  const deposit_amount = get_deposit_amount();
  if (+deposit_amount) {
    // return alert(deposit_amount);
    document.querySelector(
      "#amount_container_box",
    ).innerHTML = `DEPOSIT AMOUNT: ₦${deposit_amount
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
    return;
  }
  // errMessage_container.value = "Invalid request, please go back and try again.";
  errMessage_container.style.display = "block";
